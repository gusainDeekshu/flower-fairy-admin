...
import { 
  ...
  Layout, Columns, Database, ExternalLink,
...
export default function MenuBuilder({ slug }: { slug: string }) {
  ...
  if (loading) return (
    <div className="h-[80vh] w-full flex flex-col items-center justify-center text-slate-400 gap-4">
      ...
      <p className="text-xs font-bold uppercase tracking-widest">Initialising Menu Engine...</p>
    </div>
  );
      ...
      <div className="max-w-7xl mx-auto px-6 py-8 space-y-8">
        ...
        <section className="bg-white rounded-3xl border border-slate-200 shadow-sm  transition-all">
            <div className="bg-slate-50 px-6 py-3 border-b flex items-center gap-2">
                ...
                <span className="text-[11px] font-bold uppercase tracking-wider text-slate-500">Live Visual Preview</span>
            </div>
        ...
        <header className="sticky top-4 z-[40]">
            <div className="backdrop-blur-xl bg-white/90 border border-slate-200 rounded-2xl shadow-xl px-6 py-4 flex justify-between items-center">
                <div className="flex items-center gap-3">
                    ...
                    <div>
                        <h1 className="text-lg font-bold text-slate-900 leading-none">Navigation Builder</h1>
        ...
        <div className="space-y-10">
          {groups.map((group, gIdx) => (
            <div key={gIdx} className="bg-white rounded-[2rem] border border-slate-200 shadow-sm p-8 space-y-8">
              <div className="flex items-center justify-between border-b border-slate-50 pb-6">
                <div className="flex items-center gap-4 flex-1">
                    <span className="flex h-8 w-8 items-center justify-center rounded-lg bg-green-50 text-[#006044] font-black text-sm">{gIdx + 1}</span>
                    <input
                        value={group.title}
                        onChange={(e) => {
                            const u = [...groups];
                ...
                <div className="flex items-center gap-2">
                    ...
                    <button onClick={() => setGroups(groups.filter((_, i) => i !== gIdx))} className="h-10 w-10 flex items-center justify-center rounded-xl bg-rose-50 text-rose-500 hover:bg-rose-500 hover:text-white transition-all">
                        <Trash2 size={18} />
                    </button>
                </div>
              </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
                {(group.columns || []).map((col: any, cIdx: number) => (
                  <div key={cIdx} className="bg-slate-50 border border-slate-100 rounded-3xl p-5 space-y-5">
                    ...
                    <div className="space-y-3">
                        ...
                        <div key={iIdx} className="bg-white border border-slate-200 rounded-2xl p-4 space-y-3 shadow-sm relative group/item">
                            <div className="flex items-center justify-between">
                                <select
                                    value={item.type}
                                    onChange={(e) => {
                                        const u = [...groups];
                                        u[gIdx].columns[cIdx].items[iIdx] = { ...item, type: e.target.value, label: "", slug: "", referenceId: null };
                                        setGroups(u);
                                    }}
                                ...
                                <button onClick={() => {
                                    ...
                                    setGroups(u);
                                }} className="text-slate-200 hover:text-rose-500 transition-colors"><Trash2 size={14} /></button>
                            </div>
                            ...
                            {item.type === "COLLECTION" ? (
                                <select
                                    ...
                                    onChange={(e) => {
                                        ...
                                        if (!selected) return;
                                        const u = [...groups];
                                        u[gIdx].columns[cIdx].items[iIdx] = { ...item, label: selected.name, slug: selected.slug, referenceId: selected.id };
                                        setGroups(u);
                                    }}
                                    className="w-full border-slate-200 border rounded-xl px-3 py-2 text-sm font-medium focus:ring-2 focus:ring-[#006044] outline-none"
                                >
                        ...
                        <button onClick={() => {
                            const u = [...groups];
                            u[gIdx].columns[cIdx].items.push({ label: "", slug: "", type: "COLLECTION", referenceId: null });
                            setGroups(u);
        ...
        <div className="flex justify-center pt-8 border-t border-slate-100">
            <button
                onClick={() => setGroups([...groups, { title: "NEW NAVIGATION GROUP", columns: [] }])}
